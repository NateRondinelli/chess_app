...
def legal_squares(square):
    ...
    _board.set_piece_at(square, chess.Piece(chess.BISHOP, chess.WHITE))
...
class BishopBoard(ttk.Frame):
    def __init__(self, parent):
        ...
        self.bishop_sq = 0
        self.bishop = chess.Piece(chess.BISHOP, chess.WHITE)
        self.position = chess.Board(None)
        self.position.set_piece_at(self.bishop_sq, self.bishop)
        _x, _y = xy_from_square(self.bishop_sq)
    ...
    def update_position(self):
        _row = math.floor(self.bishop_sq / 8)
        _col = self.bishop_sq % 8
        if self.bishop_sq == 0 or self.bishop_sq == 56:
            self.up = not self.up
        if _row % 2 == 0:
        ...
        if _col == _switch:
            self.bishop_sq += 8 if self.up else -8
        else:
            _col += _increment
            self.bishop_sq = _row * 8 + _col
        self.position = chess.Board(None)
        self.position.set_piece_at(self.bishop_sq, self.bishop)
        sq_x, sq_y = xy_from_square(self.bishop_sq)
    ...
    def flash_toggle(self):
        _squares = legal_squares(self.bishop_sq)
        for _square in _squares:
            sq_id = self.square_ids[_square]
            _row = 7 - math.floor(_square / 8)
            _col = _square % 8
            _color = LIGHT_COLOR if (_row + _col) % 2 == 0 else DARK_COLOR
