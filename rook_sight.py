...
def legal_squares(square):
    ...
    _board.set_piece_at(square, chess.Piece(chess.ROOK, chess.WHITE))
...
class RookBoard(ttk.Frame):
    def __init__(self, parent):
        ...
        self.rook_sq = 0
        self.rook = chess.Piece(chess.ROOK, chess.WHITE)
        self.position = chess.Board(None)
        self.position.set_piece_at(self.rook_sq, self.rook)
        _x, _y = xy_from_square(self.rook_sq)
    ...
    def update_position(self):
        _row = math.floor(self.rook_sq / 8)
        _col = self.rook_sq % 8
        if self.rook_sq == 0 or self.rook_sq == 56:
            self.up = not self.up
        if _row % 2 == 0:
        ...
        if _col == _switch:
            self.rook_sq += 8 if self.up else -8
        else:
            _col += _increment
            self.rook_sq = _row * 8 + _col
        self.position = chess.Board(None)
        self.position.set_piece_at(self.rook_sq, self.rook)
        sq_x, sq_y = xy_from_square(self.rook_sq)
    ...
    def flash_toggle(self):
        _squares = legal_squares(self.rook_sq)
