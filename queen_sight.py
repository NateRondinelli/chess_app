...
def legal_squares(square):
    ...
    _board.set_piece_at(square, chess.Piece(chess.QUEEN, chess.WHITE))
...
class QueenBoard(ttk.Frame):
    def __init__(self, parent):
        ...
        self.queen_sq = 0
        self.queen = chess.Piece(chess.QUEEN, chess.WHITE)
        self.position = chess.Board(None)
        self.position.set_piece_at(self.queen_sq, self.queen)
        _x, _y = xy_from_square(self.queen_sq)
    ...
    def update_position(self):
        _row = math.floor(self.queen_sq / 8)
        _col = self.queen_sq % 8
        if self.queen_sq == 0 or self.queen_sq == 56:
            self.up = not self.up
        if _row % 2 == 0:
        ...
        if _col == _switch:
            self.queen_sq += 8 if self.up else -8
        else:
            _col += _increment
            self.queen_sq = _row * 8 + _col
        self.position = chess.Board(None)
        self.position.set_piece_at(self.queen_sq, self.queen)
        sq_x, sq_y = xy_from_square(self.queen_sq)
    ...
    def flash_toggle(self):
        _squares = legal_squares(self.queen_sq)
